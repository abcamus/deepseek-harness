...
export function SpeakingPractice({ materials, level, exercise, status, onStart, onSubmitted, onDiscuss, onExit }: SpeakingPracticeProps) {
  ...
  const lineCard = (line: SpeakingLine, index: number) => {
    const isPartner = line.role === 'A'
    const attempt = attempts[index]
    const recording = recordingIndex === index
    return (
      <div className={`listen-q sp-line ${recording ? 'recording' : ''}`} key={`${round.time}-${String(index)}`}>
        ...
        <div className="sp-actions">
          ...
          {!isPartner && attempt !== undefined && attempt.audioUrl !== null && (
            <button className="listen-rate" onClick={() => { handlePlayMine(index, attempt.audioUrl ?? '') }}>
              {minePlaying === index ? '🔈 播放中' : '▶ 我的录音'}
            </button>
          )}
          {!isPartner && !isAttemptDone(attempt) && (
            <button className="listen-rate" onClick={() => { markManual(index) }}>自评通过</button>
          )}
          {!isPartner && attempt !== undefined && attempt.score !== null && scoreBadge(attempt.score)}
          {!isPartner && attempt !== undefined && attempt.manual && <span className="sp-badge plain">自评通过</span>}
        </div>
    ...
    <div className="listening speaking">
      <div className="listen-panel">
        ...
        <div className="listen-questions">
          {round.sentences.map((line, i) => lineCard(line, i))}
        </div>
