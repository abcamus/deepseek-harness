...
export function App() {
  ...
  const startListening = useCallback((source: ExerciseSource) => {
    const levelHint = '先读取 .english-learning/profile.json 里的定级结果判断我的水平（没有档案再读 .english-learning/progress/ 下的记录）'
    ...
    void sendMessage(`请使用 exercise-generator 技能生成一组听力练习（来自听力练习页）。${levelHint}，然后${origin}：写一段符合我水平的英文短文，出 3-5 道四选一理解题，把完整练习（含答案和解析）写入 .english-learning/listening-session.json；聊天里只回复一句确认，不要输出原文和题目。`)
  }, [sendMessage])
  ...
  const startSpeaking = useCallback((source: ExerciseSource) => {
    ...
    const origin = source.kind === 'material'
      ? `基于学习资料「${source.material.name}」出题（用 read 工具读取文件：${source.material.path}）`
      : source.topic.trim() === '' ? '话题由你按我的水平和薄弱维度决定' : `就话题「${source.topic.trim()}」出题`
    void sendMessage(`请使用 exercise-generator 技能生成一组口语练习（来自口语练习页）。${levelHint}，然后${origin}：出 5-8 句适合朗读的英文句子（可以是跟读句，或一组 A/B 情景对话，A 是对方的话、B 是我读的话），把完整练习（含 role 标注）写入 .english-learning/speaking-session.json；聊天里只回复一句确认，不要输出练习内容。`)
  }, [sendMessage])
  ...
  const startReading = useCallback((source: ExerciseSource) => {
    ...
    const origin = source.kind === 'material'
      ? `基于学习资料「${source.material.name}」出题（用 read 工具读取文件：${source.material.path}）`
      : source.topic.trim() === '' ? '话题由你按我的水平和薄弱维度决定' : `就话题「${source.topic.trim()}」出题`
    void sendMessage(`请使用 exercise-generator 技能生成一组阅读理解练习（来自阅读练习页）。${levelHint}，然后${origin}：写一段符合我水平的英文短文，出 3-5 道四选一理解题，把完整练习（含答案和解析）写入 .english-learning/reading-session.json；聊天里只回复一句确认，不要输出原文和题目。`)
  }, [sendMessage])
  ...
  const startWriting = useCallback((source: ExerciseSource) => {
    ...
    const origin = source.kind === 'material'
      ? `基于学习资料「${source.material.name}」出题（用 read 工具读取文件：${source.material.path}）`
      : source.topic.trim() === '' ? '话题由你按我的水平和薄弱维度决定' : `就话题「${source.topic.trim()}」出题`
    void sendMessage(`请使用 exercise-generator 技能生成一篇写作练习（来自写作练习页）。${levelHint}，然后${origin}：出英文写作题目和具体要求（用词、字数、时态，targetWords 为建议词数），把写作任务写入 .english-learning/writing-session.json；聊天里只回复一句确认，不要输出题目。`)
  }, [sendMessage])
  ...
  const submitWritingEssay = useCallback((task: WritingTask, essay: string) => {
    ...
    void sendMessage(`请批改我写的作文（来自写作练习页，题目「${task.title}」，要求：${requirements}）。先读取 .english-learning/writing-session.json 里我这份任务的定级信息，按写作练习页的批改流程批改正文，完成后把结构化结果（kind 为 writing-result）重写进同一文件。我的作文正文：\n${essay}`)
  }, [sendMessage])
