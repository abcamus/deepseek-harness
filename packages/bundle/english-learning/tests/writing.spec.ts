...
describe('isWritingSessionFile', () => {
  it('matches relative and absolute launch-workspace forms', () => {
    expect(isWritingSessionFile('.english-learning/writing-session.json')).toBe(true)
    expect(isWritingSessionFile('/home/learner/ws/.english-learning/writing-session.json')).toBe(true)
    expect(isWritingSessionFile('C:\\ws\\.english-learning\\writing-session.json')).toBe(true)
  })

  it('rejects other files and look-alike directories', () => {
    expect(isWritingSessionFile('writing-session.json')).toBe(false)
    expect(isWritingSessionFile('.english-learning/reading-session.json')).toBe(false)
    expect(isWritingSessionFile('.english-learning/materials/writing-session.json')).toBe(false)
    expect(isWritingSessionFile('my.english-learning/writing-session.json')).toBe(false)
  })
})
...
describe('classifyWritingWrite', () => {
  ...
  it('classifies a task write and a result write to the session file', () => {
    const write = (content: unknown): { name: string; arguments: string } => ({
      name: 'write',
      arguments: JSON.stringify({ file_path: '.english-learning/writing-session.json', content: JSON.stringify(content) }),
    })
  ...
  it('classifies an absolute write target', () => {
    expect(classifyWritingWrite({
      name: 'write',
      arguments: JSON.stringify({ file_path: 'C:\\ws\\.english-learning\\writing-session.json', content: JSON.stringify(taskDoc) }),
  ...
  it('ignores a session-target write whose content does not parse', () => {
    expect(classifyWritingWrite({
      name: 'write',
      arguments: JSON.stringify({ file_path: '.english-learning/writing-session.json', content: '{oops' }),
    })).toEqual({ kind: 'none' })
  })
  ...
  it('ignores unrelated tools, paths, and argument shapes', () => {
    ...
    expect(classifyWritingWrite({
      name: 'write',
      arguments: JSON.stringify({ file_path: '.english-learning/reading-session.json', content: JSON.stringify(taskDoc) }),
    })).toEqual({ kind: 'none' })
    expect(classifyWritingWrite({
      name: 'write',
      arguments: JSON.stringify({ file_path: '.english-learning/writing-session.json' }),
    })).toEqual({ kind: 'none' })
  })
})
