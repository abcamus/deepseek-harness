...
describe('isListeningSessionFile', () => {
  it('matches relative and absolute launch-workspace forms', () => {
    expect(isListeningSessionFile('.english-learning/listening-session.json')).toBe(true)
    expect(isListeningSessionFile('/home/learner/ws/.english-learning/listening-session.json')).toBe(true)
    expect(isListeningSessionFile('C:\\ws\\.english-learning\\listening-session.json')).toBe(true)
  })

  it('rejects other files and look-alike directories', () => {
    expect(isListeningSessionFile('listening-session.json')).toBe(false)
    expect(isListeningSessionFile('.english-learning/profile.json')).toBe(false)
    expect(isListeningSessionFile('.english-learning/materials/listening-session.json')).toBe(false)
    expect(isListeningSessionFile('my.english-learning/listening-session.json')).toBe(false)
  })
})
...
describe('classifyListeningWrite', () => {
  ...
  it('classifies an absolute write target', () => {
    expect(classifyListeningWrite({
      name: 'write',
      arguments: JSON.stringify({ file_path: 'C:\\ws\\.english-learning\\listening-session.json', content: JSON.stringify(exerciseDoc) }),
    })).toEqual({ kind: 'exercise', exercise: exerciseDoc })
  })
  ...
  it('ignores a session-target write whose content does not parse', () => {
    expect(classifyListeningWrite({
      name: 'write',
      arguments: JSON.stringify({ file_path: '.english-learning/listening-session.json', content: '{oops' }),
    })).toEqual({ kind: 'none' })
  })
  ...
  it('ignores unrelated tools, paths, and argument shapes', () => {
    ...
    expect(classifyListeningWrite({
      name: 'write',
      arguments: JSON.stringify({ file_path: '.english-learning/listening-session.json' }),
    })).toEqual({ kind: 'none' })
  })
})
