...
describe('isReadingSessionFile', () => {
  it('matches relative and absolute launch-workspace forms', () => {
    expect(isReadingSessionFile('.english-learning/reading-session.json')).toBe(true)
    expect(isReadingSessionFile('/home/learner/ws/.english-learning/reading-session.json')).toBe(true)
    expect(isReadingSessionFile('C:\\ws\\.english-learning\\reading-session.json')).toBe(true)
  })

  it('rejects other files and look-alike directories', () => {
    expect(isReadingSessionFile('reading-session.json')).toBe(false)
    expect(isReadingSessionFile('.english-learning/listening-session.json')).toBe(false)
    expect(isReadingSessionFile('.english-learning/materials/reading-session.json')).toBe(false)
    expect(isReadingSessionFile('my.english-learning/reading-session.json')).toBe(false)
  })
})
...
describe('classifyReadingWrite', () => {
  ...
  it('classifies a write to the reading session file', () => {
    expect(classifyReadingWrite({
      name: 'write',
      arguments: JSON.stringify({ file_path: '.english-learning/reading-session.json', content: JSON.stringify(exerciseDoc) }),
  ...
  it('classifies an absolute write target', () => {
    expect(classifyReadingWrite({
      name: 'write',
      arguments: JSON.stringify({ file_path: 'C:\\ws\\.english-learning\\reading-session.json', content: JSON.stringify(exerciseDoc) }),
    })).toEqual({ kind: 'exercise', exercise: exerciseDoc })
  })
  ...
  it('ignores a session-target write whose content does not parse', () => {
    expect(classifyReadingWrite({
      name: 'write',
      arguments: JSON.stringify({ file_path: '.english-learning/reading-session.json', content: '{oops' }),
    })).toEqual({ kind: 'none' })
  })
  ...
  it('ignores unrelated tools, paths, and argument shapes', () => {
    ...
    expect(classifyReadingWrite({
      name: 'write',
      arguments: JSON.stringify({ file_path: '.english-learning/reading-session.json' }),
    })).toEqual({ kind: 'none' })
  })
})
