...
describe('isSpeakingSessionFile', () => {
  it('matches relative and absolute launch-workspace forms', () => {
    expect(isSpeakingSessionFile('.english-learning/speaking-session.json')).toBe(true)
    expect(isSpeakingSessionFile('/home/learner/ws/.english-learning/speaking-session.json')).toBe(true)
    expect(isSpeakingSessionFile('C:\\ws\\.english-learning\\speaking-session.json')).toBe(true)
  })

  it('rejects other files and look-alike directories', () => {
    expect(isSpeakingSessionFile('speaking-session.json')).toBe(false)
    expect(isSpeakingSessionFile('.english-learning/listening-session.json')).toBe(false)
    expect(isSpeakingSessionFile('.english-learning/materials/speaking-session.json')).toBe(false)
    expect(isSpeakingSessionFile('my.english-learning/speaking-session.json')).toBe(false)
  })
})
...
describe('classifySpeakingWrite', () => {
  ...
  it('classifies a write to the speaking session file', () => {
    expect(classifySpeakingWrite({
      name: 'write',
      arguments: JSON.stringify({ file_path: '.english-learning/speaking-session.json', content: JSON.stringify(exerciseDoc) }),
  ...
  it('classifies an absolute write target', () => {
    expect(classifySpeakingWrite({
      name: 'write',
      arguments: JSON.stringify({ file_path: 'C:\\ws\\.english-learning\\speaking-session.json', content: JSON.stringify(exerciseDoc) }),
    })).toEqual({ kind: 'exercise', exercise: exerciseDoc })
  })
  ...
  it('ignores a session-target write whose content does not parse', () => {
    expect(classifySpeakingWrite({
      name: 'write',
      arguments: JSON.stringify({ file_path: '.english-learning/speaking-session.json', content: '{oops' }),
    })).toEqual({ kind: 'none' })
  })
  ...
  it('ignores unrelated tools, paths, and argument shapes', () => {
    ...
    expect(classifySpeakingWrite({
      name: 'write',
      arguments: JSON.stringify({ file_path: '.english-learning/listening-session.json', content: JSON.stringify(exerciseDoc) }),
    })).toEqual({ kind: 'none' })
    expect(classifySpeakingWrite({
      name: 'write',
      arguments: JSON.stringify({ file_path: '.english-learning/speaking-session.json' }),
    })).toEqual({ kind: 'none' })
  })
})
