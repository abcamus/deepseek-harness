...
describe('isPlacementFile', () => {
  it('matches relative and absolute launch-workspace forms of a placement file', () => {
    expect(isPlacementFile('.english-learning/profile.json', 'profile.json')).toBe(true)
    expect(isPlacementFile('/home/learner/ws/.english-learning/profile.json', 'profile.json')).toBe(true)
    expect(isPlacementFile('C:\\ws\\.english-learning\\profile.json', 'profile.json')).toBe(true)
    expect(isPlacementFile('.english-learning/placement-progress.json', 'placement-progress.json')).toBe(true)
  })

  it('rejects other files and look-alike directories', () => {
    expect(isPlacementFile('profile.json', 'profile.json')).toBe(false)
    expect(isPlacementFile('.english-learning/materials/doc.md', 'profile.json')).toBe(false)
    expect(isPlacementFile('my.english-learning/profile.json', 'profile.json')).toBe(false)
    expect(isPlacementFile('.english-learning/profile.json', 'placement-progress.json')).toBe(false)
  })
})
...
describe('classifyPlacementWrite', () => {
  ...
  it('classifies a write to the placement progress file', () => {
    expect(classifyPlacementWrite({
      name: 'write',
      arguments: JSON.stringify({ file_path: '.english-learning/placement-progress.json', content: JSON.stringify(progressDoc) }),
  ...
  it('classifies a write to the profile file as a completion candidate', () => {
    expect(classifyPlacementWrite({
      name: 'write',
      arguments: JSON.stringify({ file_path: 'C:\\ws\\.english-learning\\profile.json', content: JSON.stringify(profileDoc) }),
  ...
  it('ignores a manual-source or malformed profile write so chat edits cannot fake completion', () => {
    const write = (content: unknown): string => JSON.stringify({ file_path: '.english-learning/profile.json', content: JSON.stringify(content) })
    expect(classifyPlacementWrite({
      name: 'write',
  ...
  it('ignores a progress-target write whose content does not parse', () => {
    expect(classifyPlacementWrite({
      name: 'write',
      arguments: JSON.stringify({ file_path: '.english-learning/placement-progress.json', content: '{oops' }),
    })).toEqual({ kind: 'none' })
  })
  ...
  it('ignores unrelated tools, paths, and argument shapes', () => {
    ...
    expect(classifyPlacementWrite({
      name: 'write',
      arguments: JSON.stringify({ file_path: '.english-learning/profile.json' }),
    })).toEqual({ kind: 'none' })
  })
})
