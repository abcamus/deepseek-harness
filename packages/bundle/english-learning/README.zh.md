---
description: "DeepSeek Harness 的英语学习 profile：学习资料、听说读写四个练习页面、定级测评，以及由可配置 agent 技能驱动的游戏化仪表盘。"
kind: "package-bundle"
---

# @deepseek-ai/dsh-english-learning

[English](README.md) | 中文

## 概述

本包是 [DeepSeek Harness](https://github.com/deepseek-ai/deepseek-harness) 的英语学习 profile：一个本地 Web 应用，AI 导师消化学习资料、建立词汇库，并在四个专属练习页面上训练你的听说读写。定级测评确定你的 CEFR 等级，游戏化仪表盘从真实学习记录统计 XP、连续天数和各维度正确率，每个 agent 技能都可以在设置页开关。你很少直接安装本包——`dsh` CLI 自带它，profile 引用它。
...
dsh --profile english-learning
```

首次启动会打印一个带令牌的 URL（例如 `http://127.0.0.1:4000/?token=…`）并打开浏览器。`--host`、`--port`、`--no-open` 分别改变绑定地址、端口和浏览器行为。全部学习数据——资料、词汇、进度、会话文档、语音缓存——都在启动目录的 `./.english-learning/` 下；请从专门的学习目录启动 profile。
...
导师可见的状态是 `.english-learning/` 下的 agent 写入 JSON：每条学习记录一个文件、每份资料一个词汇库、学习者档案，以及各练习轮次消费的页面会话文档。bundle 对照这些文档契约分类 agent 的 `write` 调用，并投影为浏览器 SSE 事件；练习页本地批改，经 REST 端点上报结果写入相同形态的记录。
...
- **全部学习数据明文存放在启动目录的 `./.english-learning/` 下**；像对待任何本地文件一样备份或清理该目录。
