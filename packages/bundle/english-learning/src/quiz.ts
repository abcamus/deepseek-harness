...
export function parseQuizExercise<K extends string>(content: string, kind: K): QuizExercise<K> | undefined {
  ...
  if (typeof record.passage !== 'string' || record.passage.trim() === '') return undefined
...
/**
 * Whether a write target resolves to `<workspace>/.english-learning/<fileName>`.
...
export function isSessionFile(filePath: string, fileName: string): boolean {
  ...
  return segments.length >= 2
    ...
    && segments[segments.length - 2] === '.english-learning'
}
