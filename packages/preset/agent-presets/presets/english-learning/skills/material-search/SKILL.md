...
先用 `read` 工具读取 `.english-learning/profile.json`（定级档案）：

- 当前水平：档案存在时直接用 `currentLevel` 字段
- 薄弱维度：档案存在时用 `weakSkills` 字段

没有档案时，再用 `glob` 工具列出 `.english-learning/progress/*.json` 并用 `read` 工具读取这些学习记录：
...
把正文用 `write` 工具保存为：
`.english-learning/materials/<当前毫秒时间戳>-<简短英文名>.md`
